...
public class ExcelValidator {
	 ...
	 * 4) Se sono dichiarati X turni ma se ne trovano Y dove Y > X � un errore
	 * 
	 */
			...
			if (giocatoriInClassifica != null && !giocatoriInClassifica.isEmpty()){
				...
				for (GiocatoreDTO partecipante: partecipantiEffettivi){
					...
					if (!giocatoriInClassifica.contains(rigaClassificaSonda) && !partecipante.equals(GiocatoreDTO.FITTIZIO)){
						result.add(new ExcelValidatorMessages(Scheda.CLASSIFICA_RIDOTTA, "Il giocatore con l'ID "+rigaClassificaSonda.getIdGiocatore()+" non risulta in classifica nonostante abbia giocato almeno una partita. Confermi che � stato squalificato?", Severity.WARNING));
					}
				}
			}
		}
...
//						result.add(new ExcelValidatorMessages(Scheda.ISCRITTI, "L'indirizzo email del giocatore "+giocatore+" non � indicato"));
//					}
	...
	//Qui valido solo le partite: se qualcosa non va ci sar� un'eccezione
...
//				result.add(new ExcelValidatorMessages(Scheda.TURNO, "Non risulta presente nemmeno un turno di gioco: le schede Turno devono chiamarsi \"N� Turno\" (dove N � un numero)"));
//			}
//		}else{
//			result.add(new ExcelValidatorMessages(Scheda.TURNO, "Non risulta presente nemmeno un turno di gioco: le schede Turno devono chiamarsi \"N� Turno\" (dove N � un numero)"));
//		}
